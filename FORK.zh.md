...
- 在 Wayland 下，一个很小的 `ptyxis-quake-daemon` 进程负责持有 Global Shortcuts
  portal 会话，首选按键为 `Ctrl+grave`（Ctrl 加反引号键），并不包含终端或窗口实现。
- 使用过 Quake 后，每次启动新的 Ptyxis 进程时，如果快捷键服务未运行，会自动启动
  服务，但不打开 Quake 窗口。可在首选项中关闭“随 Ptyxis 启动”；此选项与登录时
  启动相互独立。手动停止后，在同一个 Ptyxis 进程中打开新窗口不会自动重启服务。
- Quake 窗口主菜单中的“退出 Quake”会关闭该窗口并停止快捷键服务。有命令运行时
  使用现有关闭确认，取消会保留所有标签。普通窗口和两个自动启动设置均不受影响。
  隐藏按钮仍会保留终端会话。
